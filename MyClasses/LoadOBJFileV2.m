function objobject=LoadOBJFileV2(modelname, debug, preparse)
...
totalcount=0;
Lyn = [];
vertexTexcoordMap = cell(1, prevnum);
...
while 1
            ...
            case 'f' % faces
                ...
                if nvrts == 3
                    ...
                    elseif nslash==6 % vertex, textures and normals,
                        ...
                        for i = 1:3
                            vertexIndex = f1(i);
                            texcoordIndex = f1(i + 3);
                            texnum = f1(1) + f1(2) + f1(3);
                            % Update the mapping for the vertex
                            if isempty(vertexTexcoordMap{vertexIndex})
                                vertexTexcoordMap{vertexIndex} = struct('texcoord', texcoordIndex, 'texnum', texnum);
                            else
                                vertexTexcoordMap{vertexIndex} = [vertexTexcoordMap{vertexIndex},...
                                    struct('texcoord', texcoordIndex, 'texnum', texnum)];
                            end
                        end
                    elseif nslash==0
                        f1=sscanf(Lyn,'%f');
    ...
    if f3num <= 0
        ...
        if vtnum > 0
            ...
            if idxdiff~=0
                ...
                Texcoords = zeros(size(SrcTexCoords, 1), vnum);
                vtnum = vnum;
                %Map texture coordinates to vertexTexcoordMap
                for j = 1:numel(vertexTexcoordMap)

                    for k = 1:numel(vertexTexcoordMap{1, j})
                        vertextex = vertexTexcoordMap{1, j}(k).texcoord;                        
                        vertexTexcoordMap{1, j}(k).texVals = SrcTexCoords(:, vertextex);
                    end
                    % texID = Faces(1,i) + Faces(2,i) + Faces(3,i);
                    % 
                    % if texID == vertexTexcoordMap{1, i}(j).texnum 
                    %     vertexTexcoordMap{1, i}(j).texcoords = Texcoords(:, vtnum);
                    % end
                end
    ...
    objobject{meshcount}.vertexTexcoordMap = vertexTexcoordMap;  %#ok<AGROW>
